"""Seed data for new Case Studies schema (JSON-based sections).
...
CASE_STUDIES_NEW_SEED: list[dict] = [
    {
...
def seed_case_studies_new(svc: ContentDBService) -> dict:
    ...
    for case_study_data in CASE_STUDIES_NEW_SEED:
        try:
            svc.upsert_case_study_new(case_study_data)
            seeded_count += 1
    ...
    total = len(CASE_STUDIES_NEW_SEED)
...
def seed_insights_new(svc: ContentDBService) -> dict:
